# -----------------------------------------------------------
# solve the knapsack problem based on an ascendingly sorted list
#o
...
def determineBestSelection(itemlist, transportLimit):
    ...
    remainingWeight = transportLimit   # set full transport limit

    # sort item list by weight using a lambda function
    itemlist.sort(key=lambda x: x["weight"])

    # go through the item list one after the next as long as the item fits
    for item in itemlist:
        if item["weight"] <= remainingWeight:
            results.append(item)       # put item into the bag
            remainingWeight -= item["weight"] # reduce available weight
