...
void main()
{
    float alphaValue = 1.0;      // alpha value
    float ambientStrength = 0.5; // strength of ambient light
    vec3 lightColor = vec3(1.0, 1.0, 1.0);  // light colour: white
    vec3 objectColor = vec3(1.0, 0.0, 0.0); // object light: red
    
    vec2 p = (gl_FragCoord.xy - 256)/100;
    float z = 1.0 - length(p);
    
    vec3 ambient = ambientStrength * lightColor;
    
    vec3 normal = normalize(vec3(p.xy, z));
    vec3 direction = normalize(lightColor);
    ...
    gl_FragColor = vec4(result, alphaValue);
}
"""
